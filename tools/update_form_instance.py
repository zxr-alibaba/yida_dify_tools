...
class UpdateFormInstanceTool(Tool):
    def _invoke(
            self,
        ...
        try:
            ...
            use_latest_version = tool_parameters['use_latest_version']
        ...
        if use_latest_version is not None:
            form_payload["useLatestVersion"] = use_latest_version

        try:
            response = requests.put(SAVE_FORM_INSTANCE_URL, headers=form_headers, json=form_payload)
            response.raise_for_status()
            yield self.create_text_message("success")
        except requests.exceptions.RequestException as e:
            yield self.create_text_message(text=f"Failed to update form instance: {str(e)}")
